...
| `TAG_SELECTION_MODE`                | Tag review mode. Use `all` for the legacy flat tag behavior or `applicable` to make leaf/standalone tags selectable while parent tags are treated as hierarchy context.                      | No       | all                        |
...
| `TAG_PARENT_CANDIDATE_NAMES`        | Optional comma-separated list of existing tag names that may be used as parents for newly created child tags. Empty means any non-system tag can be used as a parent.                        | No       |                            |
