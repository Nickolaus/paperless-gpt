...
| `PAPERLESS_API_VERSION`             | Optional Paperless REST API version to send in the `Accept` header (for example `10`). Leave unset for the server default.                                                                    | No       |                            |
...
| `LLM_REQUESTS_PER_MINUTE`           | Maximum requests per minute for the main LLM. Applies to initial requests and retry attempts. Useful for managing API costs, provider rate limits, or local LLM load.                         | No       | 120                        |
...
| `OCR_JOB_TIMEOUT_SECONDS`           | Optional timeout for manual OCR jobs. Leave unset to disable; set a bounded value when OCR jobs must not run forever.                                                                         | No       |                            |
| `MISTRAL_OCR_MAX_RETRIES`           | Maximum retry attempts for transient Mistral OCR API failures (`429`, `500`, `502`, `503`, `504`).                                                                                            | No       | 3                          |
| `MISTRAL_OCR_BACKOFF_MAX_WAIT`      | Maximum wait between Mistral OCR retry attempts.                                                                                                                                              | No       | 10s                        |
| `MISTRAL_OCR_REQUEST_TIMEOUT_SECONDS` | Timeout in seconds for Mistral OCR processing requests.                                                                                                                                       | No       | 60                         |
| `MISTRAL_OCR_CONFIDENCE_GRANULARITY` | Optional Mistral OCR confidence-score granularity passed through to the OCR API when set.                                                                                                     | No       |                            |
| `MISTRAL_OCR_TABLE_FORMAT`          | Optional Mistral OCR table format passed through to the OCR API when set.                                                                                                                     | No       |                            |
| `MISTRAL_OCR_IMAGE_REFERENCE_MODE`  | How to handle Mistral OCR image references in markdown output. Use `preserve` to keep provider links or `strip` to remove extracted-image placeholders while recording metadata.              | No       | preserve                   |
...
| `TAG_SELECTION_MODE`                | Tag review mode. Use `all` for the legacy flat tag behavior or `applicable` to make leaf tags selectable while explicit parent candidates and parents with children are treated as hierarchy context. | No       | all                        |
| `TAG_DERIVED_PARENTS`               | Include parent tags automatically when a child tag is selected or created with a parent.                                                                                                      | No       | true                       |
| `TAG_NON_CLASSIFICATION_NAMES`      | Comma-separated workflow/system tag names to exclude from AI suggestions and classification pickers.                                                                                          | No       |                            |
| `TAG_PARENT_CANDIDATE_NAMES`        | Optional comma-separated list of existing tag names that may be used as parents for newly created child tags. In `applicable` mode, explicit parent candidates are hierarchy buckets, not normal selectable classification tags. Empty keeps standalone tags selectable and allows any non-system tag as a parent for manually created child tags. | No       |                            |
