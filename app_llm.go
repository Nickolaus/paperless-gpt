...
func cleanLLMScalar(value string) string {
	value = stripMarkdown(textsanitize.StripReasoning(value))
...
func (app *App) getSuggestedCoreMetadata(ctx context.Context, suggestionRequest GenerateSuggestionsRequest, doc Document, content string, generationContext suggestionGenerationContext, logger *logrus.Entry) (coreMetadataSuggestion, error) {
	settingsMutex.RLock()
	titleSchema := settings.TitleSchema
	settingsMutex.RUnlock()
	if titleSchema == "" {
	...
	response := stripMarkdown(textsanitize.StripReasoning(completion.Choices[0].Content))
...
func (app *App) generateSingleDocumentSuggestion(ctx context.Context, suggestionRequest GenerateSuggestionsRequest, doc Document, generationContext suggestionGenerationContext, logger *logrus.Entry) (DocumentSuggestion, error) {
	...
	if suggestionRequest.GenerateTitles || suggestionRequest.GenerateTags || suggestionRequest.GenerateCorrespondents || suggestionRequest.GenerateDocumentTypes || suggestionRequest.GenerateCreatedDate {
		metadata, err := app.getSuggestedCoreMetadata(ctx, suggestionRequest, doc, content, generationContext, docLogger)
		if err != nil {
