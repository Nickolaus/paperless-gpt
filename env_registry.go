...
var envRegistry = []EnvVar{
	...
	{Name: "MISTRAL_OCR_BACKOFF_MAX_WAIT", Category: "OCR", Secret: false, Default: "", Description: "(Mistral OCR only) Maximum backoff wait between retries, as a Go duration (e.g. `30s`). Uses the provider's built-in default when unset or invalid."},
	{Name: "MISTRAL_OCR_CONFIDENCE_GRANULARITY", Category: "OCR", Secret: false, Default: "", Description: "(Mistral OCR only) Confidence score granularity requested from the Mistral OCR API."},
	{Name: "MISTRAL_OCR_MAX_RETRIES", Category: "OCR", Secret: false, Default: "", Description: "(Mistral OCR only) Maximum retry attempts for failed Mistral OCR requests. Uses the provider's built-in default when unset or invalid."},
	{Name: "MISTRAL_OCR_REQUEST_TIMEOUT_SECONDS", Category: "OCR", Secret: false, Default: "", Description: "(Mistral OCR only) Per-request timeout in seconds for Mistral OCR API calls. Uses the provider's built-in default when unset or invalid."},
	{Name: "MISTRAL_OCR_IMAGE_REFERENCE_MODE", Category: "OCR", Secret: false, Default: "preserve", Description: "(Mistral OCR only) How to handle inline image references in Mistral OCR output: `preserve` (default) or `strip`."},
	{Name: "MISTRAL_OCR_TABLE_FORMAT", Category: "OCR", Secret: false, Default: "", Description: "(Mistral OCR only) Table output format requested from the Mistral OCR API."},
	{Name: "OCR_JOB_TIMEOUT_SECONDS", Category: "Processing & limits", Secret: false, Default: "", Description: "Optional timeout for async OCR jobs. Leave unset to disable; set a bounded value for slow local providers when jobs must not run forever."},
	...
	{Name: "PAPERLESS_CREATED_TAG_CHANGE_GROUP_IDS", Category: "Tags & automation", Secret: false, Default: "", Description: "Optional comma-separated paperless-ngx group IDs granted change permission on newly created tags."},
	{Name: "PAPERLESS_CREATED_TAG_IS_INSENSITIVE", Category: "Tags & automation", Secret: false, Default: "", Description: "Optional boolean applied to newly created tags' `is_insensitive` field in paperless-ngx. Leave unset to use paperless-ngx's default."},
	{Name: "PAPERLESS_CREATED_TAG_MATCH", Category: "Tags & automation", Secret: false, Default: "", Description: "Optional match pattern applied to newly created tags' `match` field in paperless-ngx."},
	{Name: "PAPERLESS_CREATED_TAG_MATCHING_ALGORITHM", Category: "Tags & automation", Secret: false, Default: "", Description: "Optional paperless-ngx matching algorithm ID applied to newly created tags."},
	{Name: "PAPERLESS_CREATED_TAG_OWNER_ID", Category: "Tags & automation", Secret: false, Default: "", Description: "Optional owner user ID applied to newly created tags. Set to `null` to explicitly clear the owner."},
	{Name: "PAPERLESS_CREATED_TAG_VIEW_GROUP_IDS", Category: "Tags & automation", Secret: false, Default: "", Description: "Optional comma-separated paperless-ngx group IDs granted view permission on newly created tags."},
